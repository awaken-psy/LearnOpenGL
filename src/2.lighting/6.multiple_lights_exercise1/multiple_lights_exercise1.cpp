/**
 * 练习 6.1 — 四种光照氛围(沙漠 / 工厂 / 恐怖 / 生化实验室)
 *
 * 同一个多光源场景,只改光源颜色和强度配比,就能呈现完全不同的"氛围":
 *   DESERT(沙漠)         — 暖橙色调,强方向光
 *   FACTORY(工厂)        — 冷蓝色调,聚光灯窄
 *   HORROR(恐怖)         — 几乎全黑,微弱红光
 *   BIOCHEMICAL LAB(生化)— 亮绿,实验室感
 *
 * ⚠ 下方是教程给出的【代码片段】(用旧式 glUniform 写法 + [...] 占位,非完整程序),
 *    用 #if 0 屏蔽以免报错。要运行某种氛围,把对应段落的光源参数填进
 *    6.multiple_lights.cpp 的 uniform 设置处即可。
 */
